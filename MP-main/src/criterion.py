...
class ContentPerceptualLoss(nn.Module):
    ...
    def wd(generated_images, target_images, deivces): # N,1,H,W
        ...
        for angle in [15., 30., 45., 60., 75.]:
            f1r = K.geometry.rotate(f1, angle * torch.ones(B, device=devices))
            f2r = K.geometry.rotate(f2, angle * torch.ones(B, device=deivces))
        ...
        return loss
    def calculate_loss(generated_images, target_images,deivces):
        return torch.mean(wd(generated_images.mean(dim=1, keepdim=True), target_images.mean(dim=1, keepdim=True),deivces))
